...
def get_X_y(df_train, ds):
    y = torch.tensor(df_train[ds[4][-1]].to_numpy())
    x = torch.tensor(df_train.drop(ds[4][-1], axis=1).to_numpy())
...
def get_data_split(ds, seed):
    ...
    path = f"dataset_descriptions/openml_{ds[0]}.txt"
    try:
        with open(path) as f:
            ds[-1] = f.read()
    except:
...
def load_kaggle():
    ...
    for name in kaggle_dataset_ids:
        ...
        ds = [
            "kaggle_" + name[0],
            df_all[list(set(df_all.columns) - set([name[2]]))].values,
            df_all[name[2]].values,
            [],
            df_train.columns.tolist(),
            "",
        ]
        path = f"dataset_descriptions/kaggle_{name[0]}.txt"
        try:
            with open(path) as f:
                ds[-1] = f.read()
        except:
...
kaggle_dataset_ids = [  # Format: (Dataset ID, Dataset Name, Target Column)
    (
        "health-insurance-lead-prediction-raw-data",
        "Health Insurance Lead Prediction Raw Data",
        "Response",
    ),
    ("pharyngitis", "pharyngitis", "radt"),
]
